...
export class SupuestosRiesgosService {
  ...
  getSupuestosRiesgos(page: number = 1, pageSize: number = 10, term: string = ''): Observable<ResponseI> {
    return this.helperHandler.handleRequest(() => this.http.get<ResponseI>(`${this.baseUrl}/SupuestosRiesgos?Term=${term}&CurrentPage=${page}&PageSize=${pageSize}`, this.header))
  }
