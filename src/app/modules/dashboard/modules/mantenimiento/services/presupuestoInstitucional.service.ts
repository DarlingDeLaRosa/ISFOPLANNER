...
export class PresupuestoInstitucionalService {
  ...
  public getPresupuestoInstitucional(page:number = 1,enUso?: boolean, pageSize: number = 10 ) {
    return this.helperHandler.handleRequest(() => this.http.get(`${this.baseURL}/PresupuestoInstitucional?CurrentPage=${page}&PageSize=${pageSize}&enUso=${enUso ?? ''}`, this.header))
  }
  ...
  public getUnidadesPresupuestoAsignado(page: number = 1,  pageSize: number = 10) {
    return this.helperHandler.handleRequest(() => this.http.get(`${this.baseURL}/PresupuestoInstitucional/presupuestos-asignados?CurrentPage=${page}&PageSize=${pageSize}`, this.header))
  }
